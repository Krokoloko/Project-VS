...
{
    [Signal]
    public delegate void OnLeaveMenu();
    ...
    public override void _Ready()
    {
        ...
        this.Connect("OnLeaveMenu", this, "UnbindDelegates");
    }

    private void UnbindDelegates()
    {
        var signals = GetSignalConnectionList("OnLeaveMenu");

        for (int i = 0; i < signals.Count; i++)
        {
            var connection = (Godot.Collections.Dictionary)signals[i];
            this.Disconnect("OnLeaveMenu", (Godot.Object)connection["target"], (string)connection["method"]);
        }
